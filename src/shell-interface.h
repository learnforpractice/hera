...
namespace wasm {
		...
		class Memory {
			...
			std::vector < char >memory;
			 template < typename T >
				...
				return 0 == (reinterpret_cast < uintptr_t >
					     (address) & (sizeof(T) - 1));
 ...
 public:
			 Memory() {
				...
				if (aligned < T > (&memory[address])) {
					*reinterpret_cast <
					...
					std::memcpy(&memory[address], &value,
						    sizeof(T));
				}
			}
			template < typename T > T get(size_t address) {
				if (aligned < T > (&memory[address])) {
					...
					T loaded;
					std::memcpy(&loaded, &memory[address],
						    sizeof(T));
					return loaded;
				}
			}
 ...
 ShellExternalInterface():memory() {
		...
		void init(Module & wasm, ModuleInstance & instance) override {
			memory.resize(wasm.memory.initial *
				      wasm::Memory::kPageSize);
			// apply memory segments
 for (auto & segment:	wasm.memory.segments) {
				Address offset =
				    ConstantExpressionRunner <
				    TrivialGlobalManager >
				    (instance.globals).visit(segment.offset).
				    value.geti32();
				assert(offset + segment.data.size() <=
				       wasm.memory.initial *
				       wasm::Memory::kPageSize);
				for (size_t i = 0; i != segment.data.size();
				     ++i) {
 ...
 for (auto & segment:	wasm.table.segments) {
		...
		void importGlobals(std::map < Name, Literal > &globals,
				   Module & wasm) override {
			// add spectest globals
 for (auto & import:	wasm.imports) {
				if (import->kind == ExternalKind::Global
				    && import->module == SPECTEST
				    && import->base == GLOBAL) {
 ...
 for (auto argument:		arguments) {
			...
			Fatal() << "callImport: unknown import: " << import->
			    module.str << "." << import->name.str;
		}

		Literal callTable(Index index, LiteralList & arguments,
				  WasmType result,
				  ModuleInstance & instance) override {
			if (index >= table.size())
				trap("callTable overflow");
			auto *func =
			    instance.wasm.getFunctionOrNull(table[index]);
			if (!func)
			...
			return instance.callFunctionInternal(func->name,
							     arguments);
		}

		int8_t load8s(Address addr) override {
			return memory.get < int8_t > (addr);
		}
		uint8_t load8u(Address addr) override {
			return memory.get < uint8_t > (addr);
		}
		int16_t load16s(Address addr) override {
			return memory.get < int16_t > (addr);
		}
		uint16_t load16u(Address addr) override {
			return memory.get < uint16_t > (addr);
		}
		int32_t load32s(Address addr) override {
			return memory.get < int32_t > (addr);
		}
		uint32_t load32u(Address addr) override {
			return memory.get < uint32_t > (addr);
		}
		int64_t load64s(Address addr) override {
			return memory.get < int64_t > (addr);
		}
		uint64_t load64u(Address addr) override {
